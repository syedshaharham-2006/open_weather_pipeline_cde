...
def extract_weather_data_func(**context):
    http_hook = HttpHook(method='GET', http_conn_id='weathermap_api')
    response = http_hook.run(
        endpoint='/data/2.5/weather?q=Karachi&APPID=a2e7d649697e18a8148f981dd5e3644e'
    )
    data = response.json()
...
def transform_load_data(task_instance):
    ...
    df_data.to_csv(f"s3://cde-weather-bucket/{file_name}", index=False, storage_options=aws_credentials)
...
with DAG(
    ...
    is_weather_api_ready = HttpSensor(
        ...
        endpoint='/data/2.5/weather?q=Karachi&APPID=<your api key>'
    )

    extract_weather_data_task = PythonOperator(
        task_id='extract_weather_data',
        python_callable=extract_weather_data_func
    )
